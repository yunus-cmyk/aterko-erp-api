...
function teslimatVeri(t, kullaniciAd) {
    ...
    veri['KOD'] = t.sartname_kodu || '';   // şartname kodu ({proje_kodu}-TŞ-NN) — şablonda {{KOD}}
...
function teknikSartnameHTML(t, ft, kullaniciAd) {
    ...
    // Künye satırı: yalnız DOLU alanlar yazılır. Eskiden boş kat bilgisi olan
    // konteynerlerde "- mm - Kat -" gibi boş parçalar görünüyordu (2026-08-03).
    // m² Türkçe biçimde (virgüllü) gösterilir.
    const paramParcalari = [
        t.bina_tipi,
        t.konteyner_ebadi,
        t.kat_yuksekligi ? t.kat_yuksekligi + ' mm' : null,
        t.kat_adedi ? t.kat_adedi + ' Kat' : null,
        t.buyukluk_m2 != null && t.buyukluk_m2 !== ''
            ? (parseFloat(t.buyukluk_m2)).toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' m²'
            : null
    ].filter(x => x != null && String(x).trim() !== '');
    const param = paramParcalari.length ? `[ ${paramParcalari.map(esc).join(' - ')} ]` : '';
