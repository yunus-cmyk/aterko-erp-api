...
function isEmriHTML(t, ft, kullaniciAd, emirNo, isEmriNotu, projeTeslimatlar) {
    ...
    // Formdaki koşullu gizleme: "X=Y→SORU_GIZLE" — form ekranıyla aynı
    const gizliMi = a => {
        if (!a.kosullar) return false;
    ...
    const altBolumHTML = b => {
        const rows = b.alanlar.filter(a => !gizliMi(a))
    ...
    return `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><style>
      ...
      * { box-sizing: border-box; }
      ...
      <div class="bolum-bas bolum-ana">
        ...
        <span class="turuncu">${esc(emirNo || '')} · ${esc(tarihSaat)}</span>
