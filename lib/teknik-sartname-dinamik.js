...
// =============================================================================
// İŞ EMRİ PDF gövdesi — veri kaynağı FORM TANIMLARI (şartname şablonu DEĞİL):
// oradaki başlıklar, oradaki sorular ve formda verilen cevaplar (veri[soru]).
// Yerleşim: bölüm BAŞLIĞI + altında tek kutu (tablo) — satır satır soru/cevap.
// En başta "Proje Bilgileri" ve "Teslimat Bilgileri" (proje/teslimat künyesinden),
// en sonda "İş Emri Notu". Görsel dil teknik şartname ile BİREBİR (Rubik, 8pt,
// turuncu hücre çizgileri, [NN] turuncu bölüm numarası).
// ft: form_tanimlari satırları [{bolum_adi, bolum_sirasi, soru, kosullar}]
// =============================================================================
function isEmriHTML(t, ft, kullaniciAd, emirNo, isEmriNotu) {
    const veri = teslimatVeri(t, kullaniciAd);

    // Formdaki koşullu gizleme: "X=Y→SORU_GIZLE" — form ekranındaki davranışın aynısı
    const gizliMi = a => {
        if (!a.kosullar) return false;
        return String(a.kosullar).split('||').some(k => {
            ...
            const c = String(cevap == null ? '' : cevap).trim();
            if (beklenen === 'Yok') return c === '' || c === 'Yok';
            return c === beklenen;
        });
    };
    const cevapHTML = v => {
    ...
    // Bölüm = başlık + altında tek tablo (şartname stili)
    let sayac = 0;
    const bolumHTML = (ad, rows) => {
        if (!rows) return '';
        const no = String(++sayac).padStart(2, '0');
        return `
        <div class="bolum-bas"><span class="ana"><span class="bno">[${no}]</span> ${esc(String(ad || '').toLocaleUpperCase('tr'))}</span></div>
        <table class="ts">${rows}</table>`;
    };
    const ciftler = liste => liste
        .filter(([, v]) => v != null && String(v).trim() !== '')
        .map(([s, v]) => satirHTML(esc(s), esc(String(v)))).join('');

    // 1) Proje Bilgileri + 2) Teslimat Bilgileri — künyeden (form tanımlarından DEĞİL)
    const pbHTML = bolumHTML('Proje Bilgileri', ciftler([
    ...
    const tbHTML = bolumHTML('Teslimat Bilgileri', ciftler([
    ...
    // 3) Form Tanımları bölümleri — formdaki sıra, formdaki sorular, formdaki cevaplar
    const bolumler = []; const map = {};
    ft.forEach(a => {
    ...
    bolumler.sort((x, y) => x.sira - y.sira).forEach(b => {
        ...
        const rows = b.alanlar.filter(a => !gizliMi(a)).map(a => {
            const c = cevapHTML(veri[a.soru]);
            if (!c) return '';                                   // cevaplanmamış soru iş emrine yazılmaz
            return satirHTML(esc((a.soru || '').split('\n')[0]), c);
        }).filter(Boolean).join('');
        orta += bolumHTML(b.ad, rows);
    });

    // 4) İş Emri Notu — en sonda, aynı düzen (başlık + kutu)
    const notHTML = (isEmriNotu && String(isEmriNotu).trim())
        ? bolumHTML('İş Emri Notu', `<tr><td class="cevap">${esc(String(isEmriNotu).trim()).replace(/\n/g, '<br>')}</td></tr>`)
        : '';
    ...
    return `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><style>
      ...
      * { box-sizing: border-box; }
      ...
      <div class="header"><img src="images/siparis_logo.png" alt="ATERKO"></div>
      <div class="bolum-bas bolum-ana">
        <span class="turuncu">İŞ EMRİ</span>
        <span class="turuncu">${esc(emirNo || '')} · ${esc(veri['TARİH'])}</span>
        <span class="ana">${esc((t.bina_adi || '').toLocaleUpperCase('tr'))}</span>
      </div>
      ${pbHTML}
      ${tbHTML}
      ${orta}
