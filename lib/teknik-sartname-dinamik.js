...
function teknikSartnameHTML(t, ft, kullaniciAd) {
    ...
    const cevapHTML = (a, v) => {
        ...
        if (a.cevap_sablonu != null) return cevapBicim(motorIsle(a.cevap_sablonu, veri));
        // Eski model: secenek_metinleri / kaynak_soru
    ...
    ft.forEach(a => { if (!map[a.bolum_adi]) { map[a.bolum_adi] = { ad: a.bolum_adi, sira: a.bolum_sirasi, gizle: a.bolum_gizle || null, aciklama: a.bolum_aciklama || null, alanlar: [] }; bolumler.push(map[a.bolum_adi]); } map[a.bolum_adi].alanlar.push(a); });

    let formHtml = '';
    bolumler.forEach(b => {
        // Bölüm gizleme (HARİCİ): kaynak alan "Yok"/boş ise tüm bölüm gösterilmez
        if (b.gizle) {
            const c = String(veri[b.gizle.alan] == null ? '' : veri[b.gizle.alan]).trim();
            if (b.gizle.deger === 'Yok' && (c === '' || c === 'Yok')) return;
        }
        const satir = b.alanlar
            .filter(a => a.soru !== b.ad)       // bölümün ana Var/Yok sorusu başlıkta, satır değil
            .filter(a => !gizliMi(a))           // koşulla gizlenenleri çıkar
            .map(a => {
                // Soru: ilk satır başlık, sonraki satırlar (marka vb.) küçük/gri
        ...
        formHtml += `<table class="ts">${satir}</table>`;
    });
