        ...
        matches.forEach(m => {
            ...
            if (sagladi) {
                ...
                if ($tr.length) {
                    ...
                    // Tablo dışı (Google Docs: <p>{{HARICI}}</p> bölüm başlığı tablosu ile
                    // içerik tablosu arasında). Bu bölümü komple gizle: önceki bölüm-başlığı
                    // tablosu + HARICI paragrafı + sonraki bölüm-başlığına kadar tüm içerik.
                    const $blok = $textNode.closest('p').length ? $textNode.closest('p') : $parent;
                    const bolumBasligiMi = $el => $el.is('table') &&
                        /^\d+$/.test($el.find('tr').first().children().first().text().trim());
                    const $bas = $blok.prev();
                    let $cur = $blok.next();
                    while ($cur.length) {
                        if (bolumBasligiMi($cur)) break;       // sonraki bölüm başladı → dur
                        const $rm = $cur; $cur = $cur.next(); $rm.remove();
                    }
                    if (bolumBasligiMi($bas)) $bas.remove();    // bu bölümün başlık tablosu
                    $blok.remove();                              // HARICI paragrafı
                }
            } else {
