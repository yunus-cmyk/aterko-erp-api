        ...
        matches.forEach(m => {
            ...
            if (sagladi) {
                ...
                if ($tr.length) {
                    ...
                    const bolBas = $el => $el.hasClass('bolum-h') || $el.hasClass('ana-baslik') ||
                        ($el.is('table') && /^\d+$/.test($el.find('tr').first().children().first().text().trim()));
                    // Bu bölümlerde başlık kalsın, içeriğe "Hariçtir." yazılsın (komple gizleme yerine)
                    const HARICTIR = ['Zemin', 'Altyapı', 'Alt Yapı'];
                    if (HARICTIR.includes(alan)) {
                        let $n = $blok.next(); let ilk = true;
                        while ($n.length && !bolBas($n)) {
                            const $rm = $n; $n = $n.next();
                            if (ilk) { $rm.replaceWith('<p class="c2"><span class="c19">Hariçtir.</span></p>'); ilk = false; }
                            else $rm.remove();
                        }
                        $blok.remove();
                    } else {
                        // Komple gizle: bölüm başlığı tablosu + içerik (sonraki bölüm başlığına kadar)
                        let $p = $blok.prev();
...
async function renderToPDF(templateName, degerler) {
    return htmlToPDF(renderTemplate(templateName, degerler));
}
