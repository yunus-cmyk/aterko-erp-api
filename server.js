...
app.post('/api/mali-hareket-sil', yetkiKontrol, async (req, res, next) => {
    try {
        const on = await pool.query('SELECT taraf_tip FROM cari_hareketler WHERE id=$1', [req.body.id]);
...
app.get('/api/mali-ayar', yetkiKontrol, async (req, res, next) => {
    try {
        ...
        if (!ayar.varsayilan_tahsilat_vadesi) ayar.varsayilan_tahsilat_vadesi = '2026-07-13';
        // Hesap listesi yoksa eski tekli kasa açılışından türet
        if (!Array.isArray(ayar.hesaplar) || !ayar.hesaplar.length)
            ayar.hesaplar = [{ ad: 'Kasa', acilis: parseFloat(ayar.kasa_acilis) || 0 }];
...
app.post('/api/mali-ayar', yetkiKontrol, async (req, res, next) => {
    try {
        ...
        let hesaplar = Array.isArray(req.body.hesaplar) ? req.body.hesaplar
            ...
            .filter(h => h.ad) : null;
        if (!hesaplar || !hesaplar.length) hesaplar = [{ ad: 'Kasa', acilis: parseFloat(req.body.kasa_acilis) || 0 }];
        const yeni = { muhlet_tarihi: req.body.muhlet_tarihi || null,
            kasa_acilis: hesaplar.reduce((s, h) => s + h.acilis, 0),   // eski alan = açılışlar toplamı (geriye uyum)
            hesaplar,
            varsayilan_odeme_vadesi: req.body.varsayilan_odeme_vadesi || '2026-07-13',
            varsayilan_tahsilat_vadesi: req.body.varsayilan_tahsilat_vadesi || '2026-07-13' };
...
app.post('/api/mali-hareket-planla', yetkiKontrol, async (req, res, next) => {
    try {
        const h = await pool.query('SELECT tip, taraf_tip, gerceklesen_tarih FROM cari_hareketler WHERE id=$1', [req.body.id]);
...
app.post('/api/mali-hareket-gerceklestir', yetkiKontrol, async (req, res, next) => {
    ...
    try {
        ...
        const on = await pool.query('SELECT taraf_tip FROM cari_hareketler WHERE id=$1', [id]);
        if (on.rowCount && on.rows[0].taraf_tip === 'MUSTERI' && !(await maliMusteriYetkiVar(req, 'YAZMA')))
            return res.status(403).json({ ok: false, izin_hatasi: true, hata: 'Müşteri carisi için yazma yetkiniz yok.' });
...
app.post('/api/mali-hareket-geri-al', yetkiKontrol, async (req, res, next) => {
    const cl = await pool.connect();
    try {
        const on = await pool.query('SELECT taraf_tip FROM cari_hareketler WHERE id=$1', [req.body.id]);
        if (on.rowCount && on.rows[0].taraf_tip === 'MUSTERI' && !(await maliMusteriYetkiVar(req, 'TAM')))
        ...
        await cl.query('COMMIT');
        const h = u.rows[0];
        await auditLogla(req, { eylem: 'UPDATE', tablo: 'cari_hareketler', kayit_id: parseInt(req.body.id), ozet: `GERÇEKLEŞME GERİ ALINDI: ${h.taraf_tip} #${h.taraf_id} ${h.tip} ${h.tutar} TL${d.rowCount ? ' (+otomatik karşı kayıt silindi)' : ''}` });
        res.json({ ok: true, mesaj: 'Gerçekleşme geri alındı.' + (d.rowCount ? ' Otomatik oluşan karşı kayıt da silindi.' : '') });
    } catch (e) { await cl.query('ROLLBACK').catch(() => {}); next(e); }
    finally { cl.release(); }
});

// Gerçekleşmemiş hareket düzeltme (YAZMA) — asıl belge alanları; plan alanlarına dokunmaz
app.post('/api/mali-hareket-guncelle', yetkiKontrol, async (req, res, next) => {
    try {
        const b = req.body;
        const h = await pool.query('SELECT * FROM cari_hareketler WHERE id=$1', [b.id]);
        if (!h.rowCount) return res.json({ ok: false, hata: 'Hareket bulunamadı.' });
        if (h.rows[0].taraf_tip === 'MUSTERI' && !(await maliMusteriYetkiVar(req, 'YAZMA')))
            return res.status(403).json({ ok: false, izin_hatasi: true, hata: 'Müşteri carisi için yazma yetkiniz yok.' });
...
// Tedarikçinin son siparişleri (fatura girişini siparişten doldurmak için köprü)
app.get('/api/mali-tedarikci-siparisler/:id', yetkiKontrol, async (req, res, next) => {
    try {
        const r = await pool.query(`
            SELECT s.id, s.siparis_no, s.siparis_tarihi, s.durum, COALESCE(s.para_birimi, 'TL') AS para_birimi,
                   COALESCE(SUM(sk.birim_fiyat * sk.siparis_miktari), 0) AS toplam
            FROM satinalma_siparisleri s
            JOIN siparis_kalemleri sk ON sk.siparis_id = s.id
            WHERE s.tedarikci_id = $1 AND COALESCE(s.durum, '') <> 'İPTAL'
            GROUP BY s.id ORDER BY s.siparis_tarihi DESC NULLS LAST, s.id DESC LIMIT 20`, [req.params.id]);
        res.json({ ok: true, data: r.rows });
    } catch (e) { next(e); }
});

// Kasa bakiyesi = hesap açılışları + gerçekleşen tahsilatlar − ödemeler − ödenen çekler
// Hesap bazlı döner: hareket hesabı boşsa ilk (varsayılan) hesaba sayılır
async function maliKasaBakiye() {
    const a = await pool.query("SELECT deger FROM sistem_ayarlari WHERE anahtar='mali_ayar'");
    const d = a.rows[0]?.deger || {};
    const hesapTanim = (Array.isArray(d.hesaplar) && d.hesaplar.length)
        ? d.hesaplar.map(h => ({ ad: h.ad, acilis: parseFloat(h.acilis) || 0 }))
    ...
    try {
        ...
        for (const d of alacakDevirler.rows) {
            ...
            const kalem = { id: null, taraf_tip: 'MUSTERI', taraf_id: d.id, firma_adi: d.firma_adi,
                tip: 'DEVIR', belge_no: null, cek_no: null, projeksiyon_tur: null,
                yon: 'GIRIS', tutar, asil_vade: varsayilanTahsilat, planlanan_vade: null, vade: tahsilatVade,
                tasindi: tahsilatVade !== varsayilanTahsilat };
            if (tahsilatVade <= bitis) kalemler.push(kalem);
        }
        kalemler.sort((a, b) => a.vade < b.vade ? -1 : a.vade > b.vade ? 1 : 0);
        res.json({ ok: true, bugun, bitis, kasa_bugun: kasa.bakiye, kasa_acilis: kasa.acilis, varsayilan_vade: varsayilanVade, varsayilan_tahsilat_vadesi: varsayilanTahsilat, kalemler, gecikmis, vadesiz });
    } catch (e) { next(e); }
});
...
app.post('/api/teknik-sartname-kaydet', yetkiKontrol, async (req, res, next) => {
    try {
        ...
        // SATIŞ FAZI KİLİDİ (Yunus 2026-07-29): bağlı teklif ONAYLANAN iken teknik
        // şartname kilitlidir — değişiklik ancak teklif revize edilirse (aktif teklif
        // yeniden taslağa dönünce) yapılabilir. Kabul, şartnameyi de mühürler.
        const sk = await pool.query(`
            SELECT COALESCE(p.faz,'TESLIMAT') AS faz,
                   EXISTS (SELECT 1 FROM sat_teklifler t
                           WHERE t.proje_id = p.id AND t.durum = 'ONAYLANAN') AS onayli_var
            FROM proje_teslimatlari pt
            JOIN projeler p ON p.id = pt.proje_id
            WHERE pt.id=$1`, [teslimat_id]);
        if (sk.rows[0]?.faz === 'SATIS' && sk.rows[0]?.onayli_var) {
            return res.json({ ok: false, hata: 'Teklif ONAYLANDIĞI için teknik şartname KİLİTLİDİR. Değişiklik gerekiyorsa önce teklifin revize edilmesi gerekir.' });
        }
    ...
    try {
        ...
        const temiz = kalemler.filter(k => (k.ad || '').trim()).map((k, i) => {
            ...
            return {
                ...
                montaj_gerekli: binaTuru && binaTuru !== 'Diğer' ? !!k.montaj_gerekli : null,
        ...
        if (id) {
            ...
            const kalanIdler = temiz.map(k => k.id).filter(Boolean);
            const silinecek = (await client.query(
                `SELECT id FROM sat_teklif_kalemleri WHERE teklif_id=$1 ${kalanIdler.length ? 'AND NOT (id = ANY($2::int[]))' : ''}`,
                kalanIdler.length ? [id, kalanIdler] : [id])).rows.map(r => r.id);
            if (silinecek.length) {
                // Silinen kalemin analiz verisi de silinir (eski ProposalComponentDeleter +
                // AnalysisDataDeleter birebir): önce döküm ve değerler, sonra bölümler.
                await client.query('DELETE FROM sat_analiz_urunler WHERE kalem_id = ANY($1::int[])', [silinecek]);
                await client.query(`DELETE FROM sat_analiz_degerler WHERE kalem_id = ANY($1::int[])`, [silinecek]);
                await client.query('DELETE FROM sat_analiz_bolumler WHERE kalem_id = ANY($1::int[])', [silinecek]);
                await client.query('DELETE FROM sat_yorumlar WHERE kalem_id = ANY($1::int[])', [silinecek]);
                await client.query('DELETE FROM sat_teklif_kalemleri WHERE id = ANY($1::int[])', [silinecek]);
            }
        } else {
            const ins = await client.query(`
        ...
        for (const k of temiz) {
            // Tutar kuruşa yuvarlanır (eski calculateAndSetTotalPrice birebir)
            const tutar = Math.round(k.miktar * k.birim_fiyat * 100) / 100;
...
// Birim adı → sembol (eski unit_type.symbol birebir)
const BIRIM_SEMBOL = {
    'Adet': 'adet', 'Metrekare': 'm²', 'Metreküp': 'm³', 'Kilogram': 'kg', 'Ton': 'ton',
    'Metre': 'm', 'Metrekare Başına Düşen Kilogram': 'kg/m2', 'Öğün': 'öğün', 'Kilometre': 'km',
    'Metretül': 'mtül', 'Dakika': 'dk', 'Litre': 'litre', 'Gün': 'Gün', 'Paket': 'paket',
    'Yevmiye': 'yev.', 'Yol-Ulaşım': 'km'
};

// Teklif toplamlarını kalemlerden hesaplar — eski calculateAndSaveProposalTotals birebir:
//   ara toplam = Σ ZORUNLU kalemlerin (miktar × birim fiyat)
//   kdv = ara toplam × kdv oranı / 100      genel = ara toplam + kdv
//   toplam büyüklük = ikincil birime göre gruplanmış Σ(miktar × ikincil miktar)  → "462,00 m²"
// NOT: eski sistemde saklı tutarlar bayat kalabiliyor (KDV oranı sonradan değişmiş);
// eski arayüz ekranda yeniden hesaplıyor — biz de kayıtta hesaplayıp saklıyoruz.
async function satisTeklifToplamlariHesapla(sorgulayici, teklifId) {
    const t = (await sorgulayici.query('SELECT kdv_orani FROM sat_teklifler WHERE id=$1', [teklifId])).rows[0];
    if (!t) return null;
    const kalemler = (await sorgulayici.query(
        'SELECT miktar, birim_fiyat, ikincil_miktar, ikincil_birim, ikincil_birim_sembol, opsiyonel FROM sat_teklif_kalemleri WHERE teklif_id=$1',
        [teklifId])).rows;
    const kdvOrani = parseFloat(t.kdv_orani) || 0;
    let ara = 0, opsiyonlu = 0;
    const birimToplam = {};
    for (const k of kalemler) {
        const m = parseFloat(k.miktar) || 0;
        const tutar = Math.round(m * (parseFloat(k.birim_fiyat) || 0) * 100) / 100;
        opsiyonlu += tutar;
        if (k.opsiyonel) continue;                       // yalnız zorunlu kalemler ara toplama girer
        ara += tutar;
        const sembol = k.ikincil_birim_sembol || BIRIM_SEMBOL[k.ikincil_birim] || k.ikincil_birim;
        const ikincil = parseFloat(k.ikincil_miktar);
        if (sembol && !isNaN(ikincil)) birimToplam[sembol] = (birimToplam[sembol] || 0) + m * ikincil;
    }
    const kdv = ara * kdvOrani / 100;
    const buyukluk = Object.entries(birimToplam)
        .map(([s, v]) => v.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' ' + s)
        .join(', ') || null;
    await sorgulayici.query(`UPDATE sat_teklifler SET ara_toplam=$1, kdv_tutar=$2, genel_toplam=$3,
        opsiyonlu_toplam=$4, toplam_buyukluk=$5 WHERE id=$6`,
        [ara, kdv, ara + kdv, opsiyonlu, buyukluk, teklifId]);
    return { ara_toplam: ara, kdv_tutar: kdv, genel_toplam: ara + kdv, opsiyonlu_toplam: opsiyonlu, toplam_buyukluk: buyukluk };
}
...
// Teklif silme — eski ProposalDeleterImpl → ProposalComponentDeleterImpl →
// AnalysisDataDeleterImpl zinciri birebir: kalemler, analiz dökümü/değerleri/bölümleri
// ve yorumlar da silinir; sonunda müşteri satış durumu yeniden hesaplanır.
app.delete('/api/satis-teklif-sil/:id', yetkiKontrol, izinGerekli('satis.teklif', 'TAM'), async (req, res, next) => {
    ...
    try {
        ...
        const kalemler = (await client.query('SELECT id FROM sat_teklif_kalemleri WHERE teklif_id=$1', [id])).rows.map(r => r.id);
        await client.query('BEGIN');
        if (kalemler.length) {
            await client.query('DELETE FROM sat_analiz_urunler WHERE kalem_id = ANY($1::int[])', [kalemler]);
            await client.query('DELETE FROM sat_analiz_degerler WHERE kalem_id = ANY($1::int[])', [kalemler]);
            await client.query('DELETE FROM sat_analiz_bolumler WHERE kalem_id = ANY($1::int[])', [kalemler]);
            await client.query('DELETE FROM sat_yorumlar WHERE kalem_id = ANY($1::int[])', [kalemler]);
        }
        await client.query('DELETE FROM sat_teklifler WHERE id=$1', [id]);
        await musteriDurumGuncelle(client, eskiR.rows[0].musteri_id);
        await client.query('COMMIT');
        await auditLogla(req, { eylem: 'DELETE', tablo: 'sat_teklifler', kayit_id: id,
            ozet: `Taslak teklif silindi: ${eskiR.rows[0].teklif_no} (${kalemler.length} kalem ve analiz verisiyle)`,
            eski_veri: eskiR.rows[0] });
        res.json({ ok: true });
    } catch (e) { await client.query('ROLLBACK'); next(e); }
    finally { client.release(); }
});
    ...
    try {
        ...
        const t = await pool.query(`
            SELECT t.*, m.ad AS musteri_adi, m.uzun_ad AS musteri_uzun_ad, m.adres AS musteri_adres,
                   COALESCE(p.proje_kodu::text, t.eski_proje_id::text) AS proje_kodu,
                   COALESCE(p.proje_adi, t.eski_proje_adi) AS proje_adi_goster,
                   p.sehir, p.ulke, p.irtibat_adi, p.irtibat_email, p.irtibat_telefon, p.satis_temsilcisi
            FROM sat_teklifler t
            LEFT JOIN sat_musteriler m ON m.id=t.musteri_id
            LEFT JOIN projeler p ON p.id=t.proje_id
            WHERE t.id=$1`, [id]);
        ...
        const { htmlToPDF } = require('./lib/pdf-generator');
        const tk = t.rows[0];
        // Altbilgi (teknik şartname PDF'iyle aynı dil): sol sayfa no, sağ künye,
        // ortada eski sistemden devralınan fidan notu.
        const fesc = s => String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        // Teklifteki her bina bileşeninin teknik şartnamesi belgenin sonuna eklenir
        const sartnameler = await teklifSartnameGovdeleri(id, req.user.adSoyad);
        const pdfBuffer = await htmlToPDF(satisTeklifHTML(tk, kalemler, sartnameler), {
            margin: { top: '18mm', bottom: '20mm', left: '20mm', right: '20mm' },
            displayHeaderFooter: true,
            headerTemplate: '<div></div>',
            footerTemplate: `<div style="width:100%;font-family:'Rubik','Helvetica',sans-serif;font-size:7pt;color:#888;font-style:italic;padding:0 20mm;box-sizing:border-box;display:flex;justify-content:space-between;align-items:center;">` +
                `<span><span class="pageNumber"></span> / <span class="totalPages"></span></span>` +
                `<span>${fesc(tk.proje_adi_goster || '')} // ${fesc(tk.teklif_no || '')}</span>` +
                `</div>`
        });
...
//   [01] TEKLİF DETAYLARI   → proje bilgileri + bileşen tablosu + toplamlar
//   [02] TEKLİF KOŞULLARI   → açıklamalar / ödeme / teslimat
//   [03] DAHİLLER VE HARİÇLER → kapsamda olan / olmayan hizmetler
function satisTeklifHTML(t, kalemler, sartnameGovdeleri = []) {
    const esc = s => String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const fmt = n => (parseFloat(n) || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const trh = d => d ? new Date(d).toLocaleDateString('tr-TR') : '-';
    const pb = t.para_birimi || 'TL';

    // Serbest metin alanları madde listesine çevrilir (eski çıktıdaki • düzeni).
    // Zaten "-" veya "•" ile başlayan satırların işareti tekrarlanmaz.
    const maddeler = (metin) => {
        const satirlar = String(metin || '').split('\n').map(x => x.replace(/^\s*[-•*]\s*/, '').trim()).filter(Boolean);
        if (!satirlar.length) return '';
        return `<ul class="md">${satirlar.map(s => `<li>${esc(s)}</li>`).join('')}</ul>`;
    };
    const altBolum = (baslik, metin) => metin && String(metin).trim()
        ? `<div class="alt-bas">${esc(baslik)}</div>${maddeler(metin)}` : '';

    // Kalemin künyesi: bina tipi + ölçü bilgileri (eski çıktıda ikinci satır olarak yer alıyordu)
    const kalemKunye = (k) => {
        const p = [];
        ...
        if (k.ic_duvar_kesiti) p.push('İç duvar ' + k.ic_duvar_kesiti);
        const kunye = p.join(' - ');
        // Eski kayıtlardaki 'aciklama' alanı çoğunlukla künyenin aynısıdır (yeni sistemde
        // bu alan kaldırıldı). Künye varsa açıklama tekrarlanmaz; yoksa (Diğer kalemleri)
        // tek bilgi kaynağı olduğu için gösterilir.
        const metin = kunye || (k.aciklama || '').trim();
        return metin ? `<div class="alt">${esc(metin)}</div>` : '';
    };

    const projeYeri = [t.ulke, t.sehir].filter(Boolean).join(', ');
    const hitap = (t.irtibat_adi || '').trim();
    const temsilci = (t.satis_temsilcisi || '').trim();
    ...
    return `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><style>
      ...
      * { box-sizing: border-box; }
      ...
      .baslik { color:#1a1a1a; font-weight:700; font-size:12pt; margin-bottom:10px; }
      /* Kapak üstü iki satırlık künye tablosu (eski çıktıdaki düzen): solda muhatap
         ve iletişimi, sağda tarih ve teklif numarası. Çerçevesiz, ince ayraçlı. */
      table.ustbilgi { width:100%; border-collapse:collapse; margin-bottom:16px; }
      table.ustbilgi td { padding:3px 0; font-size:8.5pt; border:none; border-bottom:1px solid #ffd9cc; }
      table.ustbilgi td.sag { text-align:right; white-space:nowrap; font-weight:700; }
      .mektup { font-size:8.5pt; line-height:1.6; margin:0 0 6px; }
      .mektup .selam { font-weight:700; margin-bottom:4px; }
      /* Bölümler ve şartnameler yeni sayfadan başlar */
      .yeni-sayfa { page-break-before: always; }
      .imza { margin-top:10px; font-size:8.5pt; line-height:1.5; }
      .imza .ad { font-weight:700; }
      .bolum-bas { margin:16px 0 6px; page-break-inside:avoid; page-break-after:avoid; }
      .bolum-bas .ana { color:#1a1a1a; font-weight:700; font-size:12pt; }
      .bolum-bas .ana .bno { color:#ff4c00; }
      .alt-bas { font-weight:700; font-size:9.5pt; margin:10px 0 4px; page-break-after:avoid; }
      ...
      table.ts td.no { width:26px; text-align:center; }
      table.ts td .alt { font-weight:400; font-size:7.5pt; color:#666; font-style:italic; }
      table.ts tr.ops td { color:#888; font-style:italic; }
      table.ts tr.toplam td { font-weight:700; background:#fff8f6; }
      table.ts tr.genel td { font-weight:700; font-size:9.5pt; background:#fff3ef; border-top:2px solid #ff4c00; }
      ul.md { margin:0 0 8px; padding-left:16px; }
      ul.md li { font-size:8pt; line-height:1.5; margin-bottom:2px; }
      /* Sona eklenen teknik şartname gövdelerinin kendi sınıfları (şartname PDF'iyle aynı) */
      .bolum-ana { display:flex; flex-direction:column; }
      .bolum-ana .turuncu { color:#ff4c00; font-weight:700; font-size:11pt; line-height:1.3; }
      .bolum-aciklama { font-weight:700; font-size:8.5pt; margin:0 0 6px; }
      table.ts td.soru { font-weight:700; width:34%; }
      ...
      <div class="baslik">FİYAT TEKLİFİ</div>
      <table class="ustbilgi">
        <tr>
          <td>${esc(hitap || t.musteri_adi || '')}</td>
          <td class="sag">${trh(t.teklif_tarihi)}</td>
        </tr>
        <tr>
          <td>${esc([t.irtibat_email, t.irtibat_telefon].filter(Boolean).join(' // '))}</td>
          <td class="sag">${esc(t.teklif_no || '')}</td>
        </tr>
      </table>

      <div class="mektup">
        ${hitap ? `<div class="selam">Sayın ${esc(hitap)},</div>` : ''}
        ${projeYeri ? `${esc(projeYeri)} bölgesinde yapılması planlanan ` : ''}<b>${esc(t.proje_adi_goster || '')}</b>
        ile ilgili fiyat teklifimiz aşağıda bilgilerinize sunulmuştur.
        Teklifimizin olumlu karşılanacağı ümidiyle değerlendirmelerinize sunarız.
        <div class="imza">Saygılarımla,<br><span class="ad">${esc(temsilci)}</span></div>
      </div>

      <div class="bolum-bas yeni-sayfa"><span class="ana"><span class="bno">[01]</span> TEKLİF DETAYLARI</span></div>
      <div class="alt-bas">PROJE BİLGİLERİ</div>
      <table class="ts">
        <tr><td class="e">Teklif No / Tarih</td><td>${esc(t.teklif_no || '-')} &nbsp;·&nbsp; ${trh(t.teklif_tarihi)}</td></tr>
        <tr><td class="e">Proje Adı</td><td>${esc(t.proje_adi_goster || '-')}</td></tr>
        ${projeYeri ? `<tr><td class="e">Proje Yeri</td><td>${esc(projeYeri)}</td></tr>` : ''}
        ${t.toplam_buyukluk ? `<tr><td class="e">Proje Büyüklüğü</td><td>${esc(t.toplam_buyukluk)}</td></tr>` : ''}
      </table>

      <div class="alt-bas">TEKLİF BİLEŞENLERİ</div>
      <table class="ts">
        <thead><tr>
          <th class="no">NO</th><th>AÇIKLAMA</th>
          <th class="s">MİKTAR</th><th class="s">BİRİM</th>
          <th class="s">BİRİM FİYAT<br>[${esc(pb)}]</th><th class="s">TOPLAM FİYAT<br>[${esc(pb)}]</th>
        </tr></thead>
        <tbody>
        ${kalemler.map((k, i) => `
          <tr class="${k.opsiyonel ? 'ops' : ''}">
            <td class="no">${i + 1}</td>
            <td><b>${esc(k.ad)}</b>${k.ikincil_miktar != null ? ` [${fmt(k.ikincil_miktar)} ${esc(k.ikincil_birim_sembol || 'm²')}]` : ''}
                ${k.opsiyonel ? '<div class="alt">Opsiyonel — toplama dahil değildir</div>' : ''}
                ${kalemKunye(k)}</td>
            <td class="s">${fmt(k.miktar)}</td>
            <td class="s">${esc(k.birim || 'adet')}</td>
            <td class="s">${fmt(k.birim_fiyat)}</td>
            <td class="s">${fmt(k.toplam)}</td>
          </tr>`).join('')}
          <tr class="toplam"><td colspan="5" class="s">ARA TOPLAM</td><td class="s">${fmt(t.ara_toplam)}</td></tr>
          <tr class="toplam"><td colspan="5" class="s">KDV (%${parseFloat(t.kdv_orani) || 0})</td><td class="s">${fmt(t.kdv_tutar)}</td></tr>
          <tr class="genel"><td colspan="5" class="s">TOPLAM</td><td class="s">${fmt(t.genel_toplam)}</td></tr>
          ${t.iskontolu_toplam ? `<tr class="toplam"><td colspan="5" class="s">İSKONTOLU TOPLAM</td><td class="s">${fmt(t.iskontolu_toplam)}</td></tr>` : ''}
        </tbody>
      </table>

      ${(t.notlar || t.odeme_kosullari || t.teslimat_kosullari) ? `
      <div class="bolum-bas yeni-sayfa"><span class="ana"><span class="bno">[02]</span> TEKLİF KOŞULLARI</span></div>
      ${altBolum('AÇIKLAMALAR', t.notlar)}
      ${altBolum('ÖDEME KOŞULLARI', t.odeme_kosullari)}
      ${altBolum('TESLİMAT KOŞULLARI', t.teslimat_kosullari)}` : ''}

      ${(t.dahil_isler || t.haric_isler) ? `
      <div class="bolum-bas"><span class="ana"><span class="bno">[03]</span> DAHİLLER VE HARİÇLER</span></div>
      ${altBolum('TEKLİF KAPSAMINDA OLAN HİZMETLER', t.dahil_isler)}
      ${altBolum('TEKLİF KAPSAMINA DAHİL OLMAYAN HİZMETLER', t.haric_isler)}` : ''}

      ${sartnameGovdeleri.map(g => `<div class="yeni-sayfa">${g}</div>`).join('')}
    </body></html>`;
}

// Teklif PDF'inin sonuna eklenecek TEKNİK ŞARTNAME gövdeleri (Yunus 2026-08-03):
// teklifteki her bina bileşeni için ayrı şartname, her biri yeni sayfada. Şartname
// PDF'iyle AYNI üreticiden (teknikSartnameHTML) gelir; tek Puppeteer çağrısında
// basılabilmesi için <body> içeriği ayıklanır. Görsel dil zaten ortak (Rubik/turuncu),
// eksik olabilecek şartname sınıfları teklif stiline eklenmiştir.
async function teklifSartnameGovdeleri(teklifId, kullaniciAd) {
    const kalemler = (await pool.query(`
        SELECT id FROM sat_teklif_kalemleri
        WHERE teklif_id=$1 AND bina_turu IS NOT NULL AND bina_turu <> 'Diğer'
        ORDER BY sira, id`, [teklifId])).rows;
    const govdeler = [];
    for (const k of kalemler) {
        try {
            const t = await sartnameKalemVerisi(k.id);
            if (!t) continue;
            const sab = await pool.query(
                "SELECT bolum_no,bolum_adi,bolum_gizle,bolum_aciklama,soru,cevap_sablonu,yeni_tablo,baslik_gizle FROM teknik_sartname_sablonu WHERE bina_turu=$1 ORDER BY bolum_no,satir_sira",
                [t.bina_turu]);
            if (!sab.rowCount) continue;   // şablonsuz bina türü (Yapısal Çelik vb.) atlanır
            const ft = sab.rows.map(x => ({
                bolum_adi: x.bolum_adi, bolum_sirasi: x.bolum_no, soru: x.soru, cevap_sablonu: x.cevap_sablonu,
                bolum_aciklama: x.bolum_aciklama, yeni_tablo: x.yeni_tablo, baslik_gizle: x.baslik_gizle,
                bolum_gizle: x.bolum_gizle || null
            }));
            const html = teknikSartnameHTML(t, ft, kullaniciAd);
            const m = html.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
            if (m) govdeler.push(m[1]);
        } catch (e) { /* bir bina şartnamesi üretilemezse teklif PDF'i yine de çıkar */ }
    }
    return govdeler;
}
...
app.post('/api/satis-proje-kaydet', yetkiKontrol, async (req, res, next) => {
    ...
    try {
        ...
        if (id) {
            const eskiR = await client.query(`SELECT * FROM projeler WHERE id=$1 AND COALESCE(faz,'TESLIMAT')='SATIS'`, [id]);
            ...
            if (eskiR.rows[0].sat_musteri_id && eskiR.rows[0].sat_musteri_id !== sat_musteri_id)
                await musteriDurumGuncelle(client, eskiR.rows[0].sat_musteri_id);
            await client.query('COMMIT');
...
app.post('/api/satis-urun-fiyat-kaydet', yetkiKontrol, async (req, res, next) => {
    ...
    try {
        ...
        await client.query('BEGIN');
        // Aynı tarihte zaten fiyat varsa engelle (eski validateProductPriceDates birebir)
        const cakisma = await client.query(`SELECT id FROM sat_urun_fiyatlar
            WHERE urun_id=$1 AND tip=$2 AND baslangic = $3::date`, [urun_id, tipNorm, bas]);
        if (cakisma.rowCount) {
            await client.query('ROLLBACK');
            return res.json({ ok: false, hata: 'Bu tarihte zaten bir fiyat kaydı var. Bir zaman aralığında tek bir fiyat olabilir.' });
        }
        // Açık (süresiz) fiyat kapatılır: bitis = yeni baslangic — arada BOŞLUK KALMAZ
        // (eski sorgu: baslangic <= tarih < bitis). Kapanmış geçmiş satırlara dokunulmaz.
        await client.query(`UPDATE sat_urun_fiyatlar SET bitis = $3::date
            WHERE urun_id=$1 AND tip=$2 AND bitis IS NULL AND baslangic <= $3::date`, [urun_id, tipNorm, bas]);
        const ins = await client.query(`
...
function satisMiktarHesapla(eslesenUrunler, bolumler, degerler, bolumMap, ortakKategoriler,
    ...
    // Java NumberUtils.isCreatable birebir: "45,67" (çoklu seçim id listesi) SAYI DEĞİLDİR.
    // Virgülü noktaya çevirmek çoklu seçimi yanlışlıkla sayıya dönüştürürdü.
    const sayiMi = (v) => /^[+-]?(\d+\.?\d*|\.\d+)$/.test(String(v).trim());
    const toplamlar = {};
    for (const d of degerler) {
        ...
        if (!sayiMi(d.deger)) continue;
        if (p.alan_tipi !== 2) continue;
        const ad = 'SUM_' + p.kod;
        toplamlar[ad] = (toplamlar[ad] || 0) + parseFloat(d.deger);
    }
    Object.keys(toplamlar).forEach(k => ortak[k] = toplamlar[k]);
    sumAdlari.forEach(ad => { if (!(ad in toplamlar)) ortak[ad] = null; });

    // Eski motor, DEĞERİ OLAN tüm bölümleri gezer (kapsam dışı olanlar dahil) — çünkü
    // Binding bölümler arası korunur ve kapsam dışı bir değer sonraki formülleri etkileyebilir.
    // Ürün hesabı yine yalnız o bölüme EŞLEŞEN ürünler için yapılır.
    const degerliBolumIdler = new Set(degerler.map(d => d.bolum_eski_id).filter(x => x != null));
    const gezilecek = [...bolumler];
    degerliBolumIdler.forEach(id => {
        if (!gezilecek.some(b => b.eski_id === id) && bolumMap[id]) gezilecek.push(bolumMap[id]);
    });
    const sonuc = eslesenUrunler.map(u => ({ ...u, miktar: null, hesap_detay: [] }));
    for (const bolum of gezilecek) {
        ...
        for (const d of erisilen) {
            ...
            const p = parametreMap[d.parametre_eski_id];
            if (!p) continue;
            // Sayısal değer → kod değişkenine yazılır; değilse (çoklu seçim "45,67" gibi) seçim dalına gider
            if (p.kod && String(p.kod).trim() && sayiMi(d.deger)) { ortak[p.kod] = parseFloat(d.deger); continue; }
...
app.delete('/api/satis-analiz-bolum-sil/:id', yetkiKontrol, izinGerekliHerhangi([['satis.teklif','YAZMA'], ['satis.urun','YAZMA']]), async (req, res, next) => {
    try {
        ...
        (await pool.query('SELECT eski_id FROM sat_analiz_bolumler WHERE kalem_id=$1 AND ust_bolum_eski_id=$2',
            [b.kalem_id, b.eski_id])).rows.forEach(x => silinecekBolumler.push(x.eski_id));
...
app.post('/api/satis-analiz-uret', yetkiKontrol, izinGerekli('satis.urun', 'YAZMA'), async (req, res, next) => {
    ...
    try {
        ...
        for (const u of r.kalemler) {
            // Eski sistemde formülü olmayan / hesaplanamayan ürün de 0 miktarla KAYDEDİLİR
            // (kullanıcı ızgarada miktarı elle girsin diye) — atlanmaz.
            if (korunan.has(u.id)) continue;
            await client.query(`
                ...
                [kalemId, u.id, u.miktar == null ? 0 : u.miktar, u.sira || 0, u.eslesme_notu || null,
...
async function satisTeslimatlariTuret(client, projeId, teklifId, req) {
    ...
    // REVİZE temizliği (eski recreateDeliverables'ın güvenli hali): bu teklifin kalem
    // kümesinde OLMAYAN kaleme bağlı teslimatlar — iş emri yoksa — silinir; böylece
    // revize sonrası yeni sözleşme çift teslimat bırakmaz.
    const kalemIdleri = kalemler.map(k => k.id);
    await client.query(`
        DELETE FROM proje_teslimatlari pt
        WHERE pt.proje_id = $1 AND pt.teklif_kalem_id IS NOT NULL
          AND NOT (pt.teklif_kalem_id = ANY($2::int[]))
          AND NOT EXISTS (SELECT 1 FROM is_emirleri ie WHERE ie.teslimat_id = pt.id)`,
        [projeId, kalemIdleri.length ? kalemIdleri : [0]]);
    for (const k of kalemler) {
...
app.post('/api/satis-teklif-revize', yetkiKontrol, izinGerekli('satis.teklif', 'YAZMA'), async (req, res, next) => {
    ...
    try {
        ...
        // Kalemler VE ANALİZLERİ kopyalanır — eski ProposalReviseServiceImpl birebir:
        // copyAttributeCategories (üst bölüm eşlemesiyle) + copyAttributes + copyComponentProducts
        ...
        for (const k of kalemler) {
            ...
            for (const b of bolumler) {
                const yb = await client.query(`
                    INSERT INTO sat_analiz_bolumler (kaynak, kalem_id, ad, sira, kapsamda, urun_kategori_eski_id)
                    VALUES ('TEKLIF_KALEMI',$1,$2,$3,$4,$5) RETURNING id`,
                    [yeniKalemId, b.ad, b.sira, b.kapsamda, b.urun_kategori_eski_id]);
                await client.query('UPDATE sat_analiz_bolumler SET eski_id = 100000000 + id WHERE id=$1', [yb.rows[0].id]);
                const yeniEski = (await client.query('SELECT eski_id FROM sat_analiz_bolumler WHERE id=$1', [yb.rows[0].id])).rows[0].eski_id;
                bolumEsleme[b.eski_id] = { id: yb.rows[0].id, eski_id: yeniEski, ust: b.ust_bolum_eski_id };
            }
            // Üst bölüm bağlarını yeni kimliklere çevir
            for (const [eskiBolumId, y] of Object.entries(bolumEsleme)) {
                if (y.ust && bolumEsleme[y.ust]) {
                    await client.query('UPDATE sat_analiz_bolumler SET ust_bolum_eski_id=$1 WHERE id=$2',
                        [bolumEsleme[y.ust].eski_id, y.id]);
                }
            }
            // 2) Öznitelik değerleri kopyalanır (bölüm bağı yeni kimliklerle)
            const degerler = (await client.query('SELECT * FROM sat_analiz_degerler WHERE kalem_id=$1', [k.id])).rows;
            for (const d of degerler) {
                const yeniBolum = bolumEsleme[d.bolum_eski_id]?.eski_id || null;
                await client.query(`INSERT INTO sat_analiz_degerler (kaynak, kalem_id, bolum_eski_id, parametre_id, deger)
                    VALUES ('TEKLIF_KALEMI',$1,$2,$3,$4)`, [yeniKalemId, yeniBolum, d.parametre_id, d.deger]);
            }
            // 3) Malzeme dökümü kopyalanır (kilitli fiyatlarıyla)
            await client.query(`
                INSERT INTO sat_analiz_urunler (kaynak, kalem_id, urun_id, miktar, sira, notu,
                    kilit_maliyet, kilit_satis, kilit_para_birimi, kilit_tarihi, elle_duzenlendi)
                SELECT 'TEKLIF_KALEMI', $1, urun_id, miktar, sira, notu,
                       kilit_maliyet, kilit_satis, kilit_para_birimi, kilit_tarihi, elle_duzenlendi
                FROM sat_analiz_urunler WHERE kalem_id=$2`, [yeniKalemId, k.id]);
        }
        await satisTeklifToplamlariHesapla(client, yeniId);
        await client.query('COMMIT');
...
app.post('/api/satis-sozlesme-olustur', yetkiKontrol, izinGerekli('satis.teklif', 'TAM'), async (req, res, next) => {
    ...
    try {
        ...
        const t = await client.query(`
            ...
            ORDER BY guncelleme DESC`, [projeId]);
        if (!t.rowCount) {
            return res.json({ ok: false, hata: 'Bu projede onaylanmış bir teklif bulunmuyor. Önce bir teklifi onaylamalısınız.' });
        }
        // Eski findApprovedProposalByProjectId: birden fazla onaylı teklif varsa HATA verir
        if (t.rowCount > 1) {
            return res.json({ ok: false, hata: `Bu projede ${t.rowCount} adet onaylanmış teklif var. Sözleşme oluşturmak için yalnız biri onaylı kalmalıdır.` });
        }
...
async function semaGuvence() {
    try {
        ...
        const REFERANS_TOHUM = [
            ...
            ['birim', '5', 'Ton', null, 5], ['birim', '6', 'Metre', null, 6],
            ['birim', '7', 'Metrekare Başına Düşen Kilogram', null, 7], ['birim', '8', 'Öğün', null, 8],
        ...
        await pool.query(`ALTER TABLE sat_teklifler
            ADD COLUMN IF NOT EXISTS sartname_turu TEXT,
            ADD COLUMN IF NOT EXISTS toplam_buyukluk TEXT,
            ADD COLUMN IF NOT EXISTS opsiyonlu_toplam NUMERIC`).catch(() => {});
        // Birim sembolleri (eski unit_type.symbol) — "Teklif Edilen Proje Büyüklüğü" bunu kullanır
        await pool.query(`ALTER TABLE sat_teklif_kalemleri
            ADD COLUMN IF NOT EXISTS ikincil_birim_sembol TEXT`).catch(() => {});
